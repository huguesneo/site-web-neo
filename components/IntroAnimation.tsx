...
export default function IntroAnimation() {
  ...
  useEffect(() => {
    ...
    // Intro raccourcie (~1,2s) pour limiter l'impact LCP de l'accueil.
    const t1 = setTimeout(() => setPhase('zoom'), 750);
                ...
                {PIECES.map((p, i) => (
                  <g key={i} transform={p.transform}>
                    <motion.path
                      ...
                      transition={{
                        pathLength: { delay: p.delay, duration: 0.4, ease: [0.65, 0, 0.35, 1] },
                        fillOpacity: { delay: p.delay + 0.25, duration: 0.25, ease: 'easeOut' },
                      }}
                    />
                  </g>
                ))}
