...
export default function IntroAnimation() {
  ...
  useEffect(() => {
    ...
    const t2 = setTimeout(() => setShow(false), 3850);
    return () => {
      clearTimeout(t1);
      clearTimeout(t2);
        ...
        >
          <div className="absolute inset-0 flex items-center justify-center" style={{ perspective: 1600 }}>
            ...
            <motion.div
              ...
              transition={{ duration: 1.15, ease: [0.6, 0, 0.4, 1] }}
            >
                ...
                {PIECES.map((p, i) => (
                  <g key={i} transform={p.transform}>
                    <motion.path
                      ...
                      transition={{
                        pathLength: { delay: p.delay, duration: 0.8, ease: [0.65, 0, 0.35, 1] },
                        fillOpacity: { delay: p.delay + 0.5, duration: 0.45, ease: 'easeOut' },
                      }}
                    />
                  </g>
                ))}
